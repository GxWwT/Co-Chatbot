...
def extract_llm_kwargs(http_headers: dict[str, str]) -> dict[str, Any]:
    llm_kwargs = {}
    if "temperature" in http_headers:
        llm_kwargs["temperature"] = float(http_headers["temperature"])
    if "max_tokens" in http_headers:
        llm_kwargs["max_tokens"] = int(http_headers["max_tokens"])
    return llm_kwargs
